    ...
    <form onSubmit={onSubmit} className="space-y-2">
      ...
      {garments.map((g, index) => (
        <div className="grid gap-2 md:grid-cols-[2fr_1fr_1fr_auto]" key={index}>
          ...
          <input className="rounded-md border border-slate-300 px-3 py-2" type="number" min="1" value={g.quantity} onChange={(e) => onGarmentChange(index, "quantity", e.target.value)} required />
          <input className="rounded-md border border-slate-300 px-3 py-2" type="number" min="0" value={g.price} onChange={(e) => onGarmentChange(index, "price", e.target.value)} required />
